"""draw.io projection adapter (V2.3-F10, docs/v2/30, docs/v2/46).

mxGraph XML (draw.io / diagrams.net) for manual refinement: deterministic
grid layout, architecture metadata preserved in the node style, editable
without losing the source of truth (the world regenerates with --force).
"""
...
class DrawioAdapter:
    ...
    supported_intents = frozenset(
        {"technical_diagram"} & set(IntentType.__args__))  # type: ignore[attr-defined]
    version = "0.1.0"
    ...
    def project(self, intent: VisualIntent,
        ...
        mxfile = ET.Element("mxfile", {
            "host": "archskillkit", "version": self.version})
        ...
        for index, element in enumerate(elements):
            ...
            cell = ET.SubElement(root, "mxCell", {
                "id": node_id,
                "value": (f"{element['name']} · {element['kind']}"
                          f" · {element['origin']}"
            ...
            ET.SubElement(cell, "mxGeometry", {
                ...
                "width": "180", "height": "60", "as": "geometry",
            })
        ...
        for index, relation in enumerate(relations):
            ...
            if not src or not dst:
                continue
            cell = ET.SubElement(root, "mxCell", {
                "id": f"e{index}", "value": relation["kind"],
