...
| V2.2 Product Evolution | Proposed: `docs/v2/48-v2.2-product-evolution.md` | Visión: architecture intelligence **derivada con evidencia**, gobernada y servida a agentes. CLI objetivo: `ark status` (inferencia de estado + sugerencias), `ark view` (visualizadores como procesos gestionados), `ark ask` (context packs para LLM), `ark history`/`trend` + **Fitness Score** medible, `ark gate` (governance-as-code CI), `ark serve` (control plane local con diagrama embebido + evidencia navegable), `ark mcp` (distribución a Claude/Cursor), `ark schema` (autodescripción). Landscape validado (sep 2026): LikeC4 MCP sirve DSL manual; code-graph MCPs sirven símbolos; nadie ofrece derivado+evidencia+governance+context-packs. SonarQube cobra ~$30k/año por drift detection. Milestones M1-M5 con acceptance criteria; 4 decisiones abiertas antes de M1 (alias `ark`, dep MCP SDK, control plane stdlib-first, marca). |
