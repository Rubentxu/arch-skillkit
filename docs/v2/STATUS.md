...
| Gate UAT V2.1 | Partial: 1/16 PASS | UAT2-001 ejecutada y verificada (runner-only, 3/3 hashes admitidos, checkout-before == checkout-after). Los 15 escenarios restantes necesitan v2-orchestrator (11) o benchmark-harness (1) que aún no existen; UAT2-009/010 comparten patrón runner + orchestrator. Plan: `docs/v2/uat/v2.1-plan.yaml` + sessions registry + templates. |

## Camino siguiente

1. **Cerrar el gate V2.1:** ejecutar el [plan UAT](uat/v2.1-plan.yaml), consolidando sus hashes de evidencia. UAT2-017 y el KPI del Context Compiler ya tienen evidencia para la carga canónica, pero no sustituyen los UAT obligatorios. **Estado parcial:** 1/16 PASS (UAT2-001). Para los 15 restantes hace falta un `v2-orchestrator` que envuelva `archskillkit.world` / `promotion` / `proposals` / `codeindex` / `context` y un `benchmark-harness` para UAT2-017. Esa construcción es trabajo de varias sesiones; este commit deja el tracer bullet (runner → evidence → manifest → cobertura) operativo para no rediscover la tubería en cada nueva sesión.
