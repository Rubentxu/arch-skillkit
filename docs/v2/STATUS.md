...
| V2.4 Architecture Intelligence & Agent Governance | M0 Implemented | Especificación incorporada en [`49-v2.4-summary.md`](49-v2.4-summary.md) (docs 49–69, ADRs 0032–0045). **M0 Product Kernel completo** (commits `b4b6bbd`…`37f49e6`): ArchitectureSnapshot versionado con digest estable, RunLedger con store seam, RuntimeRegistry con orphan cleanup, ports query/governance, GetStatus/Explain, handlers CLI `status`/`explain`, coverage/unknowns baseline. Gates verificados: snapshot reproducible, PID/runtime fuera del event log, CI verde, CLI legacy sin regresión. Siguiente: M1 Daily Driver/Viewers. |
...
6. **V2.4 Architecture Intelligence & Agent Governance — M0 Implemented:** especificación incorporada (docs 49–69, ADRs 0032–0045); gate pre-M0 cerrado (baseline Next.js re-validado, Axum/Django planificados). M0 kernel implementado: snapshot, ledger, registry, ports, GetStatus/Explain, handlers CLI, coverage baseline. Pendiente: M1 (Daily Driver & Viewers) y sesiones OSS Axum/Django.
