...
| Gate UAT V2.1 | Implemented: 16/16 PASS (local-disk/artifacts gitignored — no fresh-clone reproducibility) | Cobertura completa F-01..F-05. UAT2-001 (runner), UAT2-002/003/018 (F-01 store isolation), UAT2-004/005/006 (F-02 event log/replay/provenance/review), UAT2-007/008 (F-03 Context Compiler budgets + read policy), UAT2-017 (F-03 benchmark-harness: source_file_reads + peak bytes), UAT2-009/010/011 (F-04 LikeC4/Arrows regen + deterministic drift), UAT2-012/013/014 (F-05 fork/diff/promote con policy gate). Pipeline: `scripts/uat/{uat.sh,v2-orchestrator.py,check-coverage.py}`. Plan + registry: `docs/v2/uat/`. **Evidencia en `artifacts/uat/v2.1/` — gitignored, no parte del clone fresco.** |
...
1. **Gate V2.1 cerrado (local-disk only):** el [plan UAT](uat/v2.1-plan.yaml) está completo (16/16 PASS). El `v2-orchestrator` envuelve `archskillkit.world` / `promotion` / `proposals` / `codeindex` / `context`; el `benchmark-harness` mide `source_file_reads` y `peak_bytes` del Context Compiler; el `check-coverage.py` valida hashes de evidencia contra disco. **Evidencia en `artifacts/uat/v2.1/` — gitignored, no reproducible desde clone fresco.** Próximo paso: cortar v0.4.0 con el gate cerrado como evidencia del release.
...
ArchitectureWorldPort implemented at `python/src/archskillkit/ports.py:17`; ARC violations 17 → 0 resolved.
CodeGraphQueryPort remains absent from the codebase (see ADR-0049).
Migration of `AnalyzeImpact` et al. to `CodeGraphQueryPort` deferred per [Slice 4 of 86-v2.5-migration-plan.md](./86-v2.5-migration-plan.md#6-slice-4--codegrah-port) to a follow-on cycle.
